/**
 * 包含n个mutations的对象
 * */

  export const RECEIVE_ADDRESS ='receive_address'; //接收地址
  export const RECEIVE_FOOD_LIST='receive_food_list';//食品分类
  export const RECEIVE_SHOPS ='receive_shops';//商店信息
  export const RECEIVE_USER_INFO ='receive_user_info';//用户信息
  export const REST_USER_INFO ='rest_user_info';//注册用户信息

  export const RECEIVE_GOODS ='receive_goods';//mock商品列表
  export const RECEIVE_INFO ='receive_info';//注册用户信息
  export const RECEIVE_RATINGS ='receive_ratings';//mock用户评价


  export const INCREMENT_FOOD_COUNT ='increment_food_count';//购物车添加食品数量
  export const DECREMENT_FOOD_COUNT ='decrement_food_count';//购物车减少食品数量

  export const CLEAR_SHOP_CART ='clear_shop_cart';//清空购物车


